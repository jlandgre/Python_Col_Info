...
class TestParseRowMajorTbl1:
    ...
    def xtest_SetColumnOrder(self, row_maj_tbl1):
        """
        Set column order based on .col_order Series
        JDL 4/22/25
        """
        ...
        # Set col_map (keys are keep columns; values are rename names)
        #row_maj_tbl1.tbl.import_col_map = \
        #    {'idx_raw':'idx', 'col #1':'col_1', 'col #2':'col_2'}
        row_maj_tbl1.SubsetCols()
        row_maj_tbl1.RenameCols()

        # Series to set column order
        lst_expected = ['idx', 'col_2', 'col_1']
        row_maj_tbl1.tbl.col_order = pd.Series(index=[0, 1, 2], data=lst_expected)
        row_maj_tbl1.SetColumnOrder()

        #check that columns were reordered
        assert list(row_maj_tbl1.df_block.columns) == lst_expected

    def xtest_RenameCols(self, row_maj_tbl1):
        """
    ...
    def xtest_SubsetCols(self, row_maj_tbl1):
        """
        Use tbl.import_col_map to subset columns based on header
        JDL 3/4/24; Modified 9/26/24; Modified 4/21/25
        """
        ...
        # Set col_map to just 2 (keys are keep columns; values are rename names)
        row_maj_tbl1.tbl.import_col_map = {'idx_raw':'idx', 'col #1':'col_1'}
        row_maj_tbl1.SubsetCols()

        # Assert that column names are correct before renaming
        lst_expected =['idx_raw', 'col #1']
        assert list(row_maj_tbl1.df_block.columns) == lst_expected
